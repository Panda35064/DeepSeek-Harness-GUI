...
Function PageLeaveReinstall
  ...
  ;   1 => 覆盖安装（删除旧版本）：先静默卸载旧版本，再继续安装
  ;   0 => 卸载此软件：运行卸载器后退出安装程序
  ${If} $R1 = 1
    Goto reinst_uninstall
  ${EndIf}

  reinst_uninstall_only:
    ; 仅卸载：运行卸载器（带界面），卸载完成后退出安装程序
    HideWindow
    ClearErrors
    ...
    ${If} $0 = 1 ; User cancelled uninstaller, return to the page
      Abort
    ${ElseIf} $0 <> 0
  ...
  reinst_uninstall:
    ...
    ${Else}
      ReadRegStr $4 SHCTX "${MANUPRODUCTKEY}" ""
      ReadRegStr $R1 SHCTX "${UNINSTKEY}" "UninstallString"
      StrCpy $R1 "$R1 /S" ; append silent uninstall
      StrCpy $R1 "$R1 _?=$4" ; append uninstall directory
      ExecWait '$R1' $0
    ${EndIf}
